...
def run(args: argparse.Namespace) -> int:
    ...
    print(
        f"  {'Filtro':<12} {'%Caótico':>8} {'%Periódico':>10} {'%Divergente':>10}  {'λ_max méd.':>10}  {'λ_max máx.':>10}"
    )
    print("  " + "-" * 62)
    ...
    for ft in ("lowpass", "highpass", "bandpass", "bandstop"):
        ...
        if n_chaotic > 0:
            ...
            print(
                f"             cutoff transição: {valid[0]:.4f}–{valid[-1]:.4f}  (mediana: {np.median(valid):.4f})"
            )
    ...
    for rank, row in enumerate(best_chaos_preserving(data_dir, top_n=5), start=1):
        print(
            f"  {rank}. {row['window']:<15} / {row['filter_type']:<10}  {row['pct_chaotic']:>5.1f}%  λ_med={row['mean_lmax']:.4f}"
        )

    _hr("PARÂMETROS ÓTIMOS (maior λ_max finito)")
    for rank, row in enumerate(optimal_parameters(data_dir, top_n=5), start=1):
        print(
            f"  {rank}. {row['window']:<15} / {row['filter_type']:<10}  ordem={row['order']:>3}  ωc={row['cutoff']:.4f}  λ_max={row['lmax']:.6f}"
        )
    ...
    print(
        f"  • {best_ft} é o filtro que mais preserva caos ({cmp[best_ft]['mean_pct_chaotic']}% da grade)"
    )
    print(f"  • {worst_ft} é o que mais suprime ({cmp[worst_ft]['mean_pct_chaotic']}% caótico)")
    print(
        f"  • Correlação ordem × λ_max: {corr['order_vs_lmax']:+.4f} (ordens maiores {'aumentam' if corr['order_vs_lmax'] > 0 else 'reduzem'} λ_max)"
    )
    print(
        f"  • Correlação cutoff × λ_max: {corr['cutoff_vs_lmax']:+.4f} (cutoffs maiores {'aumentam' if corr['cutoff_vs_lmax'] > 0 else 'reduzem'} λ_max)"
    )
    print(
        "  • Assimetria positiva nas distribuições → caudas longas de λ_max alto (caos forte em ilhas)"
    )
