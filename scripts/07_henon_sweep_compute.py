...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--quick", action="store_true", help="Run a tiny sweep for smoke-testing (~seconds)"
    )
    p.add_argument(
        "--data-dir",
        default="data/sweeps",
    ...
    p.add_argument(
        "--save", action="store_true", help="(accepted for CLI consistency; output is always saved)"
    )
    p.add_argument(
        "--no-display",
        dest="no_display",
